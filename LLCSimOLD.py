...
def sim_setting3(pos, space, v, rel_v, alpha, i, step, lane):
    ...
    while (lane < i-1):
        temp_space = position[step, lane] - pos
        if (temp_space < 200):
            break
        else:
            lane += 1
    if (lane >= i-1):
        return traci.vehicle.getAcceleration(str(i))
    else:
...
while step<=max_iter:
        ...
        if True:
            for i in range(N_VEHICLES):
                if i == 0 or i == N_lane_1 or i == N_lane_1+N_lane_2:
                    #traci.vehicle.setSpeed("%d" % i, lead_v[step//10])
                    traci.vehicle.setSpeed("%d" % i, lead_v[step])
                    #traci.vehicle.setSpeed("%d" % i, speed[step,i])
                    ...
                    if i < N_lane_1:
                        ...
                        acceleration = model1Car(space, step, v, rel_v, rel_acc, i, laneBound)
                        #acceleration = model2Cars(space, step, v, rel_v, rel_acc, i, laneBound)
                        #acceleration = model3Cars(space, step, v, rel_v, rel_acc, i, laneBound)
                        #acceleration = sim_setting3(position[step,i], space, v, rel_v, 0.6, i, step, laneBound)
    ...
    if step == pltcp:
        ...
        plt.plot(position[:pltcp, 5])
        #plt.xlim(500)
        plt.show()
        plt.plot(speed[:pltcp, 5])
        #plt.xlim(500)
        plt.show()

        plt.plot(accel[:pltcp,5])
        #plt.xlim(500)
        plt.show()
    step += 1
